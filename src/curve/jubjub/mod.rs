...
/// The fixed-base comb table for the generator, decoded at compile time from
/// the statically embedded [`COMB_TABLE`] (generated by `sage/comb.sage`).
///
/// `COMB_TABLE[i][d]` holds the affine `(x, y)` of `(d + 1) · 16^i · B`; the
/// table places those at window indices `1..=15`, with index `0` set to the
/// identity so a zero digit selects the neutral element. `n · B` is then one
/// constant-time table lookup and one complete addition per 4-bit window of
/// `n`, with no point doublings.
#[cfg(feature = "table")]
fn generator_comb() -> &'static [[Point; 16]; COMB_WINDOWS] {
    static V: [[Point; 16]; COMB_WINDOWS] = {
        const IDENTITY_WINDOW: [Point; 16] = [Point::IDENTITY; 16];
        let mut windows = [IDENTITY_WINDOW; COMB_WINDOWS];
        let mut w = 0;
        while w < COMB_WINDOWS {
            let mut d = 0;
            while d < 15 {
                let x = FieldElement::from_bytes_unchecked_be(&COMB_TABLE[w][d].0);
                let y = FieldElement::from_bytes_unchecked_be(&COMB_TABLE[w][d].1);
                // same as `Point::from_affine(&x, &y)`, in const context
                windows[w][d + 1] = Point {
                    t: x.mul_const(&y),
                    x,
                    y,
