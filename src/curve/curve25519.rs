...
impl Point {
    ...
    /// The same complete addition, with the right-hand operand given in the
    /// precomputed [`CachedPoint`] form: `2d·T2`, `Y2 - X2` and `Y2 + X2` come
    /// straight from the table instead of being recomputed.
...
fn generator_comb() -> &'static [[CachedPointAffine; COMB_DIGITS]; COMB_WINDOWS] {
    static V: [[CachedPointAffine; COMB_DIGITS]; COMB_WINDOWS] = {
        const IDENTITY_WINDOW: [CachedPointAffine; COMB_DIGITS] =
            [CachedPointAffine::IDENTITY; COMB_DIGITS];
        let mut windows = [IDENTITY_WINDOW; COMB_WINDOWS];
        let mut w = 0;
        while w < COMB_WINDOWS {
            let mut d = 0;
            while d < COMB_DIGITS {
                let (y_minus_x, y_plus_x, t2d) = &COMB_TABLE[w][d];
                windows[w][d] = CachedPointAffine {
