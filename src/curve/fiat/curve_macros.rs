...
macro_rules! fiat_define_weierstrass_points {
    ($FE:ident) => {
        ...
        /// Fixed-base comb table for the generator, decoded at compile time
        /// from the statically embedded `COMB_TABLE` (generated by
        /// `sage/comb.sage`).
        ///
        /// `COMB_TABLE[i]` holds the 15 affine points `1·B, 2·B, …, 15·B`
        /// (where `B = 16^i·G` for window `i`) as `(x, y)` coordinate byte
        /// arrays. Each window places those at indices `1..=15`, with index `0`
        /// set to the point at infinity so a zero digit selects the neutral
        /// element.
        #[cfg(feature = "table")]
        fn generator_comb() -> &'static [[projective::Point<$FE>; 16]; COMB_WINDOWS] {
            static V: [[projective::Point<$FE>; 16]; COMB_WINDOWS] = {
                const INFINITY_WINDOW: [projective::Point<$FE>; 16] =
                    [projective::Point::<$FE>::INFINITY; 16];
                let mut windows = [INFINITY_WINDOW; COMB_WINDOWS];
                let mut w = 0;
                while w < COMB_WINDOWS {
                    let mut d = 0;
                    while d < 15 {
                        let (x, y) = &COMB_TABLE[w][d];
                        windows[w][d + 1] = projective::Point {
                            x: $FE::from_bytes_unchecked(x),
                            y: $FE::from_bytes_unchecked(y),
                            z: <$FE as $crate::curve::field::Field>::ONE,
                        };
                        d += 1;
                    }
                    w += 1;
