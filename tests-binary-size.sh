#!/usr/bin/env bash
#
# binary-size.sh — dump the machine-code / data size of each part of eccoxide.
#
# eccoxide is a library crate carved up into per-algorithm cargo features
...
build_and_measure() {
    ...
    rm -f "$DEPS_DIR"/libeccoxide-*.rlib 2>/dev/null
    ...
    local rlib
    rlib="$(ls -t "$DEPS_DIR"/libeccoxide-*.rlib 2>/dev/null | head -1)"
    if [ -z "$rlib" ]; then
        echo "FAIL 0 0 0"
        return
    fi
...
# ---- formatted table ----
echo
echo "eccoxide binary size report"
